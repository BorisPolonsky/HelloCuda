...
void cpuArraySum(float *, float *, float *, const int);
void checkCpuMalloc(float *);
void checkGpuMalloc(cudaError_t);
int arrEqual(float*, float*, int);
__global__ void gpuArraySum(float *, float *, float *, const int);
...
int main()
{
	...
	cpuArraySum(h_x1, h_x2, h_y1, num);
	//Perform array sum in GPU. 
	dim3 block(2, 2, 2);
	dim3 grid(2, 2, 2);
	gpuArraySum<<<grid, block>>>(d_x1, d_x2, d_y, num);
	cudaDeviceSynchronize(); //Explicitly wait for all kernels to complete. 
...
void cpuArraySum(float *x1, float *x2, float *y, int num)
{
	for(int i = 0; i < num; i++)
	{
		y[i] = x1[i] +x2[i];
...
__global__ void gpuArraySum(float *x1, float*x2, float *y, const int n)
{
